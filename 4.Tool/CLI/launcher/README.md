...
`launcher`는 HandStack 시작 시 현재 설치 버전과 서버 manifest를 비교하고, 필요하면 현재 프로세스에서 직접 패키지를 적용한 뒤 `ack`를 올리는 진입점입니다.

`ack` 자체에는 업데이트 전용 HTTP 엔드포인트를 두지 않고, 업데이트 책임은 launcher에만 둡니다.
...
```powershell
dotnet run --project 4.Tool/CLI/launcher/launcher.csproj -- `
...
- 사용자는 `ack` 대신 `launcher`를 진입점으로 실행합니다.
- 업데이트가 필요하면 launcher가 잠금/검증/적용을 직접 수행합니다.
- 업데이트 적용이 성공하면 launcher가 최종적으로 `ack`를 올립니다.
