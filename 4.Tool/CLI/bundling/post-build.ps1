#!/usr/bin/env pwsh
#
# bundling CLI 도구 빌드 후 배포 스크립트
#
# 설명:
#   bundling CLI 도구를 dotnet publish로 빌드하여
...
dotnet publish bundling.csproj --configuration $ConfigurationMode --arch $ArchMode --os $OsMode --output $cliOutput

if ($LASTEXITCODE -ne 0) {
    Write-Error "bundling CLI 배포 실패"
    exit 1
}