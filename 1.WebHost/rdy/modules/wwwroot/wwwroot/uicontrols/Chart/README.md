...
`Chart` 폴더에는 이름이 비슷하지만 서로 완전히 다른 **두 개의 차트 엔진**이 들어 있습니다. 폴더 하나에 두 컨트롤이 공존하는 형태이므로, 어떤 파일을 쓰느냐에 따라 태그와 사용법이 완전히 달라집니다.
...
**주의**: 이 문서는 두 파일의 실제 소스 코드를 직접 읽고 확인한 내용을 바탕으로 작성되었습니다. `$chart`(Highcharts 기반)는 qcn.groupware 등에서 실사용 예시를 찾지 못해 소스 코드만으로 문서화했고, 몇 가지 **미완성/버그로 보이는 부분**이 있습니다. 자세한 내용은 `API.md`의 "참고" 절과 각 메서드 설명을 꼭 확인하세요.
