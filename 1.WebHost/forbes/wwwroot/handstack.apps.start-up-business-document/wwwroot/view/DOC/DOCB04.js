...
let $DOCB04 = {
    ...
    event: {
        ...
        btnPreview_click() {
            ...
            var purposeRatio = syn.$l.get('txtPurposeRatio').value.trim();
            var importance = syn.$l.get('txtImportance').value.trim();
            var goalResult = syn.$l.get('txtGoalResult').value.trim();
            var note = syn.$l.get('txtNote').value.replaceAll('\n', '<br />').trim();
            var createdAt = syn.$l.get('txtCreatedAt').value.trim();
            var creatorName = syn.$l.get('txtCreatorName').value.trim();
            var templateHtml = syn.$l.get('tplTPLB04').innerHTML;
