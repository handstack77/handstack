    ...
    public class DbClientLoggerClient
    {
        ...
        public DbClientLoggerClient(Serilog.ILogger logger, Serilog.ILogger? transactionLogger = null)
        {
            ...
            circuitBreakerPolicy = Policy
                ...
                {
                    logger.Error("[{LogCategory}] " + $"DbClientLoggerClient CircuitBreaker Error Reason: {iRestResponse.Result.Content}", "CircuitBreaker/onBreak");
                },
                onReset: (context) =>
                {
                    logger.Information("[{LogCategory}] " + $"DbClientLoggerClient CircuitBreaker Reset, DateTime={DateTime.Now}", "CircuitBreaker/onReset");
                });
        }
