#!/usr/bin/env pwsh
#
# transact 모듈 빌드 후 배포 스크립트
#
# 설명:
#   transact 트랜잭션 처리 모듈을 MSBuild로 빌드한 뒤,
#   빌드 출력물을 HANDSTACK_HOME/modules/transact로 미러 복사합니다.
#
# 사전 조건:
...
# 경로 설정
$transactCsproj = [System.IO.Path]::Combine($env:HANDSTACK_SRC, "2.Modules", "transact", "transact.csproj")
$buildOutput = [System.IO.Path]::Combine("bin", "Debug", "net10.0")
$deployTarget = [System.IO.Path]::Combine($env:HANDSTACK_HOME, "modules", "transact")

# (선택) 실행 중인 서버를 중지한 뒤 빌드하려면 아래 주석을 해제하세요.
# pm2 stop ack

# transact 모듈 MSBuild 빌드
Write-Host "transact 모듈 빌드 중: $transactCsproj"
dotnet msbuild $transactCsproj
if ($LASTEXITCODE -ne 0) {
    Write-Error "transact 모듈 빌드 실패"
    exit 1
}

# 빌드 출력물을 modules/transact로 미러 복사
