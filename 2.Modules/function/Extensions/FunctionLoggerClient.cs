...
{
    public class FunctionLoggerClient : IDisposable
    {
            ...
            circuitBreakerPolicy = Policy
                ...
                .CircuitBreaker(
                    ...
                    onBreak: (result, timespan) =>
                    {
                        ...
                        logger.Error($"[CircuitBreaker/onBreak] FunctionLoggerClient Circuit opened for {timespan.TotalSeconds}s. Reason: {result.Result?.Content}");
                    },
                    onReset: () =>
                    {
                        breakDateTime = null;
                        logger.Information($"[CircuitBreaker/onReset] FunctionLoggerClient Circuit reset at {DateTime.Now}");
                    },
                    onHalfOpen: () =>
                    {
        ...
        private void LogLocally(LogMessage logMessage, string logType)
        {
            ...
            if (logType == "Request")
            {
                logger.Warning($"{logType} GlobalID: {logMessage.GlobalID}, {JsonConvert.SerializeObject(logMessage)}");
            }
