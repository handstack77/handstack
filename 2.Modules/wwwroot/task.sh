...
if [ "$TASK_COMMAND" == "copy" ]; then
    ...
    rsync -av $WORKING_PATH/Contracts/ $HANDSTACK_HOME/modules/wwwroot/Contracts/
    rsync -av $WORKING_PATH/wwwroot/ $HANDSTACK_HOME/modules/wwwroot/wwwroot/
fi

if [ "$TASK_COMMAND" == "www" ]; then
    rsync -av --exclude=$WORKING_PATH/wwwroot/lib --exclude=syn.config.json $WORKING_PATH/wwwroot/ $HANDSTACK_HOME/modules/wwwroot/wwwroot/
fi
