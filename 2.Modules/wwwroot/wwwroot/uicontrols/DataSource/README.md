...
DataSource는 화면에 아무것도 그리지 않는(`display: none`) **데이터 저장 노드**입니다. `<syn_data>` 태그로 선언하면 페이지 안에 이름이 있는 자바스크립트 객체(또는 배열) 저장소 하나가 만들어지고, 다른 컨트롤(TextBox, CheckBox, Grid 등)이 여기에 "바인딩"되어 값을 주고받습니다.
