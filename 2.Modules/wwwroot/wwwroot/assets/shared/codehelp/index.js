...
let $index = {
    ...
    event: {
        grdCodeList_afterOnCellDoubleClick(event, coords, td) {
            ...
            if (coords.row > -1) {
                ...
                if (codeConfig.isMultiSelect == false) {
                    var physicalRowIndex = $grid.getPhysicalRowIndex($this.prop.gridID, coords.row);
                    var item = $grid.getSourceDataAtRow($this.prop.gridID, physicalRowIndex);
                    var code = {
                    ...
                    var physicalRowIndex = $grid.getPhysicalRowIndex($this.prop.gridID, coords.row);
                    var item = $grid.getSourceDataAtRow($this.prop.gridID, physicalRowIndex);
                    $grid.setDataAtCell($this.prop.gridID, coords.row, 'IsSelect', item.IsSelect == '1' ? '0' : '1');
                }
            }
        },
        ...
        btnConfirm_click() {
            ...
            if (codeConfig.isMultiSelect == false) {
                var previousRow = $grid.getGridValue($this.prop.gridID) == null ? -1 : $grid.getGridValue($this.prop.gridID).previousRow;
                if (previousRow == null || previousRow == undefined) {
                    previousRow = -1;
                }

                if (previousRow > -1) {
                    var physicalRowIndex = $grid.getPhysicalRowIndex($this.prop.gridID, previousRow);
                    var item = $grid.getSourceDataAtRow($this.prop.gridID, physicalRowIndex);
                    var code = {
                        value: item[$this.prop.codeConfig.CodeColumnID],
                        text: item[$this.prop.codeConfig.ValueColumnID]
                ...
                result = [];
                var length = $grid.countRows($this.prop.gridID);

                for (var rowIndex = 0; rowIndex < length; rowIndex++) {
                    var physicalRowIndex = $grid.getPhysicalRowIndex($this.prop.gridID, rowIndex);
                    var item = $grid.getSourceDataAtRow($this.prop.gridID, physicalRowIndex);
    ...
    method: {
        search() {
            ...
            if (search == '') {
                $grid.loadData($this.prop.gridID, $this.prop.codeConfig.DataSource);
            }
            else {
                ...
                $grid.loadData($this.prop.gridID, items);
            }
        ...
        initialize() {
            ...
            $grid.updateSettings($this.prop.gridID, settings);
            ...
            if (codeConfig.isAutoSearch == true) {
                ...
                if (codeConfig.isMultiSelect == false) {
                    var count = $grid.countRows($this.prop.gridID);
                    if (count == 1) {
                        var item = $grid.getSourceDataAtRow($this.prop.gridID, 0);
                    ...
                    if (0 < count) {
                        if (syn.$l.get('ddlSearchType').value == '1') {
                            ...
                            for (var i = 0; i < count; i++) {
                                var item = $grid.getSourceDataAtRow($this.prop.gridID, i);
                                var codeData = item[codeConfig.CodeColumnID];
                                if (codeData) {
                                    if (searchItems.includes(item[codeConfig.CodeColumnID].toString()) == true) {
                                        $grid.setDataAtCell($this.prop.gridID, i, 'IsSelect', '1');
                                    }
                                }
                                else {
