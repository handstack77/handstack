...
namespace forwarder.Areas.forwarder.Controllers
{
    ...
    public class ForwardProxyLabController : BaseController
    {
        ...
        {
            var cookieValue = Request.Cookies.TryGetValue("ProxyLabCookie", out var currentCookie) == true ? currentCookie : "(없음)";
            var html =
            $$"""
            <!DOCTYPE html>
            <html>
            <head>
                ...
                <title>포워드 프록시 브라우저 대상</title>
                <link rel="stylesheet" href="assets/theme.css" />
            </head>
            <body>
                <main class="lab-shell">
                    <header class="lab-hero">
                        <img class="lab-badge" src="assets/badge.svg?variant=hero" alt="포워드 프록시" />
                        <div>
                            <p class="lab-kicker">브라우저 대상</p>
                            <h1>포워드 프록시 브라우저 페이지</h1>
                            <p class="lab-copy">이 페이지는 상대 경로 CSS, JavaScript, SVG, 탐색 링크, 폼 전송을 포함해 포워더를 거친 base 태그 동작을 확인할 수 있도록 구성했습니다.</p>
                        </div>
                    </header>
                    <section class="lab-grid">
                        ...
                        <article class="lab-panel">
                            <h2>상대 경로 탐색</h2>
                            <p><a class="lab-link" href="pages/relative?origin=html-entry">상대 경로 페이지 열기</a></p>
                            <p><a class="lab-link" href="../forward-proxy-lab/target/json?origin=html-link">JSON 대상 열기</a></p>
                            <p><a class="lab-link" href="cookie/read">쿠키 대상 확인</a></p>
                        </article>
                    </section>
                    <section class="lab-panel">
                        <h2>상대 경로 폼</h2>
                        <form method="post" action="echo?origin=html-form" class="lab-form">
                            <label for="message">전송 본문</label>
                            <input id="message" name="message" value="hello-from-browser-target" />
                            <button type="submit">상대 경로 Echo POST</button>
                        </form>
                    </section>
                    <section class="lab-panel">
                        <h2>스크립트 확인</h2>
                        <div id="widget-status" data-endpoint="../forward-proxy-lab/target/json?origin=widget-script">widget.js 로딩 대기 중</div>
                    </section>
                </main>
        ...
        public ContentResult TargetRelativePage([FromQuery] string? origin = null)
        {
            ...
            <head>
                <meta charset="utf-8" />
                <meta name="viewport" content="width=device-width, initial-scale=1" />
                <title>포워드 프록시 상대 경로 페이지</title>
                <link rel="stylesheet" href="../assets/theme.css" />
            </head>
            <body>
                <main class="lab-shell">
                    <section class="lab-panel">
                        <p class="lab-kicker">상대 경로 페이지</p>
                        <h1>상대 경로 리소스 확인</h1>
                        <p class="lab-copy">유입 경로: {{origin ?? "알 수 없음"}}</p>
                        <p><a class="lab-link" href="../html?step=returned-from-relative">HTML 대상으로 돌아가기</a></p>
                        <img class="lab-badge" src="../assets/badge.svg?variant=relative" alt="상대 경로 배지" />
                    </section>
                </main>
            </body>
            </html>
            ...
            (() => {
                ...
                fetch(endpoint, {
                    ...
                    .then((data) => {
                        ...
                        panel.textContent = `widget.js 상대 경로 JSON 로드 완료: ${data.scenario} / cookie=${data.cookieValue}`;
                    })
                    .catch((error) => {
                        panel.textContent = `widget.js 로드 실패: ${error}`;
                    });
            })();
            """;
        ...
        {
            var label = string.IsNullOrWhiteSpace(variant) == true ? "프록시" : variant;
            var svg =
            $$"""
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 240" role="img" aria-label="포워드 프록시">
                <defs>
        ...
        public IActionResult TargetJson([FromQuery] string? origin = null)
        {
            return new JsonResult(new
            {
                ...
                cookieValue = Request.Cookies.TryGetValue("ProxyLabCookie", out var value) == true ? value : "(없음)",
        ...
        {
            var redirectUrl = $"{Request.GetBaseUrl()}/forwarder/api/forward-proxy-lab/target/html?step=redirect-arrived";
