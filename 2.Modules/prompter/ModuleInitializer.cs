            ...
            if (module != null)
            {
                ...
                if (File.Exists(moduleSettingFilePath) == true)
                {
                    ...
                    if (moduleConfigJson != null)
                    {
                        ...
                        ModuleConfiguration.DefaultPromptResultFieldID = string.IsNullOrWhiteSpace(moduleConfig.DefaultPromptResultFieldID) == true ? "PromptResult" : moduleConfig.DefaultPromptResultFieldID;
                        ...
                        ModuleConfiguration.AllowedKernelPlugins.Clear();
                        if (moduleConfig.AllowedKernelPlugins != null && moduleConfig.AllowedKernelPlugins.Count > 0)
                        {
                            foreach (var item in moduleConfig.AllowedKernelPlugins)
                            {
                                ModuleConfiguration.AllowedKernelPlugins.Add(item);
                            }
                        }

                        ModuleConfiguration.AllowedMcpServers.Clear();
                        if (moduleConfig.AllowedMcpServers != null && moduleConfig.AllowedMcpServers.Count > 0)
                        {
                            foreach (var item in moduleConfig.AllowedMcpServers)
                            {
                                ModuleConfiguration.AllowedMcpServers.Add(item);
                            }
                        }

                        ModuleConfiguration.AllowedCliTools.Clear();
                        if (moduleConfig.AllowedCliTools != null && moduleConfig.AllowedCliTools.Count > 0)
                        {
                            foreach (var item in moduleConfig.AllowedCliTools)
                            {
                                ModuleConfiguration.AllowedCliTools.Add(item);
                            }
                        }
                        ...
                        {
                            foreach (var item in moduleConfig.AllowedBodyFileBasePaths)
                            {
                ...
                services.AddTransient<OpenAICompatibleChatClient>();
                services.AddTransient<LMStudioChatClient>();
                services.AddTransient<ClaudeChatClient>();
