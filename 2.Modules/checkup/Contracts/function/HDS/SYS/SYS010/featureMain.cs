        ...
        protected DataSet? LF02(List<DynamicParameter> dynamicParameters, DataContext dataContext)
        {
            ...
            using (DataSet? result = new DataSet())
            {
                ...
                if (string.IsNullOrWhiteSpace(userWorkID)
                    ...
                    || string.IsNullOrWhiteSpace(parentMenuID)
                    || string.IsNullOrWhiteSpace(parentMenuName))
                {
                    result.BuildExceptionData("Y", "Warning", "필수 요청 정보 확인 필요", typeMember);
                    goto TransactionException;
                }
        ...
        protected DataSet? IF01(List<DynamicParameter> dynamicParameters, DataContext dataContext)
        {
            ...
            try
            {
                ...
                if (Directory.Exists(appBasePath) == true)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sourceItemPath) && Directory.Exists(sourceItemPath) == false)
                    {
        ...
        protected DataSet? IF02(List<DynamicParameter> dynamicParameters, DataContext dataContext)
        {
            ...
            try
            {
                ...
                if (Directory.Exists(appBasePath) == true)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sourceItemPath) && System.IO.File.Exists(sourceItemPath) == false)
                    {
                        string? sourceText = LZStringHelper.DecompressFromBase64(compressBase64);
        ...
        protected DataSet? DF01(List<DynamicParameter> dynamicParameters, DataContext dataContext)
        {
            ...
            try
            {
                ...
                if (Directory.Exists(appBasePath) == true)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sourceItemPath) && Directory.Exists(sourceItemPath) == true)
                    {
        ...
        private void BuildFileMenu(string userWorkID, string applicationID, string projectType, string searchPattern, List<Menu> menus, DirectoryInfo directory, int level)
        {
            ...
            if (projectType == "F")
            {
                foreach (var directoryInfo in directory.GetDirectories("*", SearchOption.TopDirectoryOnly))
                {
                    ...
                    foreach (var file in directoryInfo.GetFileInfos(SearchOption.AllDirectories, searchPattern.Split("|").Where(x => !string.IsNullOrWhiteSpace(x)).ToArray()))
                    {
