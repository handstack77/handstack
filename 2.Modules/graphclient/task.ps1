#!/usr/bin/env pwsh
#
# graphclient 모듈 빌드/배포 자동화 스크립트
#
# 설명:
#   HandStack 기반 graphclient 모듈의 빌드, 배포, 파일 동기화, 인증서 관리 등
...
# copy - Contracts와 wwwroot를 HANDSTACK_HOME으로 복사합니다.
#   Contracts → contracts, modules/graphclient/Contracts
#   wwwroot → modules/graphclient/wwwroot
if ($TaskCommand -eq "copy") {
    ...
    $contractsModuleDst = [System.IO.Path]::Combine($env:HANDSTACK_HOME, "modules", "graphclient", "Contracts")
    $wwwSrc = [System.IO.Path]::Combine($WorkingPath, "wwwroot")
    $wwwDst = [System.IO.Path]::Combine($env:HANDSTACK_HOME, "modules", "graphclient", "wwwroot")
