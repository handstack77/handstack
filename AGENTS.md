...
- `rdy` Windows x64 Debug 출력 복사본: `1.WebHost\rdy\publish.bat [출력 디렉터리] [publish|build]`. 기본 `publish`는 framework-dependent single-file 결과를, `build`는 RID 지정 rebuild 결과를 대상 디렉터리에 미러링한다. publish 루트의 전이 `Contracts`, `Prompts`, `Settings`, `SQL`은 제거하고 `modules/*` 내부 자산은 유지한다.
